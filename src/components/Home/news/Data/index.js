// News

export const DataSKST = [
  {
    id: 1,
    image: require('../../../../../media/img/sukiensaptoi_img_1.jpg'),
    title: '60 GIÂY, - VẼ Đ...',
    date: 'Thứ 2, 20/09/2021',
    dress: 'Tòa nhà T...',
  },
  {
    id: 2,
    image: require('../../../../../media/img/sukiensaptoi_img_1.jpg'),
    title: '60 GIÂY, - VẼ Đ...',
    date: 'Thứ 2, 20/09/2021',
    dress: 'Tòa nhà T...',
  },
  {
    id: 3,
    image: require('../../../../../media/img/sukiensaptoi_img_1.jpg'),
    title: '60 GIÂY, - VẼ Đ...',
    date: 'Thứ 2, 20/09/2021',
    dress: 'Tòa nhà T...',
  },
  {
    id: 4,
    image: require('../../../../../media/img/sukiensaptoi_img_1.jpg'),
    title: '60 GIÂY, - VẼ Đ...',
    date: 'Thứ 2, 20/09/2021',
    dress: 'Tòa nhà T...',
  },
  {
    id: 5,
    image: require('../../../../../media/img/sukiensaptoi_img_1.jpg'),
    title: '60 GIÂY, - VẼ Đ...',
    date: 'Thứ 2, 20/09/2021',
    dress: 'Tòa nhà T...',
  },
  {
    id: 6,
    image: require('../../../../../media/img/sukiensaptoi_img_1.jpg'),
    title: '60 GIÂY, - VẼ Đ...',
    date: 'Thứ 2, 20/09/2021',
    dress: 'Tòa nhà T...',
  },
];

export const DataNB = [
  {
    id: 1,
    date: '09/06/2023',
    title: 'Thông báo về việc nghỉ lễ 30/4 và 1/5',
    image: require('../../../../../media/img/sukiensaptoi_img_1.jpg'),
  },
  {
    id: 2,
    date: '09/06/2023',
    title: 'Thông báo về việc nghỉ lễ 30/4 và 1/5',
    image: require('../../../../../media/img/sukiensaptoi_img_1.jpg'),
  },
  {
    id: 3,
    date: '09/06/2023',
    title: 'Thông báo về việc nghỉ lễ 30/4 và 1/5',
    image: require('../../../../../media/img/sukiensaptoi_img_1.jpg'),
  },
  {
    id: 4,
    date: '09/06/2023',
    title: 'Thông báo về việc nghỉ lễ 30/4 và 1/5',
    image: require('../../../../../media/img/sukiensaptoi_img_1.jpg'),
  },
  {
    id: 5,
    date: '09/06/2023',
    title: 'Thông báo về việc nghỉ lễ 30/4 và 1/5',
    image: require('../../../../../media/img/sukiensaptoi_img_1.jpg'),
  },
  {
    id: 6,
    date: '09/06/2023',
    title: 'Thông báo về việc nghỉ lễ 30/4 và 1/5',
    image: require('../../../../../media/img/sukiensaptoi_img_1.jpg'),
  },
];

export const DataHT = [
  {
    title:
      'THÔNG BÁO VỀ VIỆC NHẬN BẰNG TỐT NGHIỆP (ĐỢT TỐT NGHIỆP THÁNG 06/2023)',
    imgTit: require('../../../../../media/img/notification_file_25px.png'),
    post: 'Người đăng: nhuntq20',
    imgND: require('../../../../../media/img/teacher_HT_25px.png'),
    date: 'Thời gian: 20/07/2023 11:47',
    imgDate: require('../../../../../media/img/time_25px.png'),
  },
  {
    title:
      'THÔNG BÁO VỀ VIỆC NHẬN BẰNG TỐT NGHIỆP (ĐỢT TỐT NGHIỆP THÁNG 06/2023)',
    imgTit: require('../../../../../media/img/notification_file_25px.png'),
    post: 'Người đăng: nhuntq20',
    imgND: require('../../../../../media/img/teacher_HT_25px.png'),
    date: 'Thời gian: 20/07/2023 11:47',
    imgDate: require('../../../../../media/img/time_25px.png'),
  },
  {
    title:
      'THÔNG BÁO VỀ VIỆC NHẬN BẰNG TỐT NGHIỆP (ĐỢT TỐT NGHIỆP THÁNG 06/2023)',
    imgTit: require('../../../../../media/img/notification_file_25px.png'),
    post: 'Người đăng: nhuntq20',
    imgND: require('../../../../../media/img/teacher_HT_25px.png'),
    date: 'Thời gian: 20/07/2023 11:47',
    imgDate: require('../../../../../media/img/time_25px.png'),
  },
  {
    title:
      'THÔNG BÁO VỀ VIỆC NHẬN BẰNG TỐT NGHIỆP (ĐỢT TỐT NGHIỆP THÁNG 06/2023)',
    imgTit: require('../../../../../media/img/notification_file_25px.png'),
    post: 'Người đăng: nhuntq20',
    imgND: require('../../../../../media/img/teacher_HT_25px.png'),
    date: 'Thời gian: 20/07/2023 11:47',
    imgDate: require('../../../../../media/img/time_25px.png'),
  },
  {
    title:
      'THÔNG BÁO VỀ VIỆC NHẬN BẰNG TỐT NGHIỆP (ĐỢT TỐT NGHIỆP THÁNG 06/2023)',
    imgTit: require('../../../../../media/img/notification_file_25px.png'),
    post: 'Người đăng: nhuntq20',
    imgND: require('../../../../../media/img/teacher_HT_25px.png'),
    date: 'Thời gian: 20/07/2023 11:47',
    imgDate: require('../../../../../media/img/time_25px.png'),
  },
  {
    title:
      'THÔNG BÁO VỀ VIỆC NHẬN BẰNG TỐT NGHIỆP (ĐỢT TỐT NGHIỆP THÁNG 06/2023)',
    imgTit: require('../../../../../media/img/notification_file_25px.png'),
    post: 'Người đăng: nhuntq20',
    imgND: require('../../../../../media/img/teacher_HT_25px.png'),
    date: 'Thời gian: 20/07/2023 11:47',
    imgDate: require('../../../../../media/img/time_25px.png'),
  },
  {
    title:
      'THÔNG BÁO VỀ VIỆC NHẬN BẰNG TỐT NGHIỆP (ĐỢT TỐT NGHIỆP THÁNG 06/2023)',
    imgTit: require('../../../../../media/img/notification_file_25px.png'),
    post: 'Người đăng: nhuntq20',
    imgND: require('../../../../../media/img/teacher_HT_25px.png'),
    date: 'Thời gian: 20/07/2023 11:47',
    imgDate: require('../../../../../media/img/time_25px.png'),
  },
];

export const DataHD = [
  {
    title:
      'THÔNG BÁO VỀ VIỆC NHẬN BẰNG TỐT NGHIỆP (ĐỢT TỐT NGHIỆP THÁNG 06/2023)',
    imgTit: require('../../../../../media/img/notification_file_25px.png'),
    post: 'Người đăng: nhuntq20',
    imgND: require('../../../../../media/img/teacher_HT_25px.png'),
    date: 'Thời gian: 20/07/2023 11:47',
    imgDate: require('../../../../../media/img/time_25px.png'),
  },
  {
    title:
      'THÔNG BÁO VỀ VIỆC NHẬN BẰNG TỐT NGHIỆP (ĐỢT TỐT NGHIỆP THÁNG 06/2023)',
    imgTit: require('../../../../../media/img/notification_file_25px.png'),
    post: 'Người đăng: nhuntq20',
    imgND: require('../../../../../media/img/teacher_HT_25px.png'),
    date: 'Thời gian: 20/07/2023 11:47',
    imgDate: require('../../../../../media/img/time_25px.png'),
  },
  {
    title:
      'THÔNG BÁO VỀ VIỆC NHẬN BẰNG TỐT NGHIỆP (ĐỢT TỐT NGHIỆP THÁNG 06/2023)',
    imgTit: require('../../../../../media/img/notification_file_25px.png'),
    post: 'Người đăng: nhuntq20',
    imgND: require('../../../../../media/img/teacher_HT_25px.png'),
    date: 'Thời gian: 20/07/2023 11:47',
    imgDate: require('../../../../../media/img/time_25px.png'),
  },
  {
    title:
      'THÔNG BÁO VỀ VIỆC NHẬN BẰNG TỐT NGHIỆP (ĐỢT TỐT NGHIỆP THÁNG 06/2023)',
    imgTit: require('../../../../../media/img/notification_file_25px.png'),
    post: 'Người đăng: nhuntq20',
    imgND: require('../../../../../media/img/teacher_HT_25px.png'),
    date: 'Thời gian: 20/07/2023 11:47',
    imgDate: require('../../../../../media/img/time_25px.png'),
  },
  {
    title:
      'THÔNG BÁO VỀ VIỆC NHẬN BẰNG TỐT NGHIỆP (ĐỢT TỐT NGHIỆP THÁNG 06/2023)',
    imgTit: require('../../../../../media/img/notification_file_25px.png'),
    post: 'Người đăng: nhuntq20',
    imgND: require('../../../../../media/img/teacher_HT_25px.png'),
    date: 'Thời gian: 20/07/2023 11:47',
    imgDate: require('../../../../../media/img/time_25px.png'),
  },
  {
    title:
      'THÔNG BÁO VỀ VIỆC NHẬN BẰNG TỐT NGHIỆP (ĐỢT TỐT NGHIỆP THÁNG 06/2023)',
    imgTit: require('../../../../../media/img/notification_file_25px.png'),
    post: 'Người đăng: nhuntq20',
    imgND: require('../../../../../media/img/teacher_HT_25px.png'),
    date: 'Thời gian: 20/07/2023 11:47',
    imgDate: require('../../../../../media/img/time_25px.png'),
  },
];

export const DataHP = [
  {
    title:
      'THÔNG BÁO VỀ VIỆC NHẬN BẰNG TỐT NGHIỆP (ĐỢT TỐT NGHIỆP THÁNG 06/2023)',
    imgTit: require('../../../../../media/img/notification_file_25px.png'),
    post: 'Người đăng: nhuntq20',
    imgND: require('../../../../../media/img/teacher_HT_25px.png'),
    date: 'Thời gian: 20/07/2023 11:47',
    imgDate: require('../../../../../media/img/time_25px.png'),
  },
  {
    title:
      'THÔNG BÁO VỀ VIỆC NHẬN BẰNG TỐT NGHIỆP (ĐỢT TỐT NGHIỆP THÁNG 06/2023)',
    imgTit: require('../../../../../media/img/notification_file_25px.png'),
    post: 'Người đăng: nhuntq20',
    imgND: require('../../../../../media/img/teacher_HT_25px.png'),
    date: 'Thời gian: 20/07/2023 11:47',
    imgDate: require('../../../../../media/img/time_25px.png'),
  },
  {
    title:
      'THÔNG BÁO VỀ VIỆC NHẬN BẰNG TỐT NGHIỆP (ĐỢT TỐT NGHIỆP THÁNG 06/2023)',
    imgTit: require('../../../../../media/img/notification_file_25px.png'),
    post: 'Người đăng: nhuntq20',
    imgND: require('../../../../../media/img/teacher_HT_25px.png'),
    date: 'Thời gian: 20/07/2023 11:47',
    imgDate: require('../../../../../media/img/time_25px.png'),
  },
  {
    title:
      'THÔNG BÁO VỀ VIỆC NHẬN BẰNG TỐT NGHIỆP (ĐỢT TỐT NGHIỆP THÁNG 06/2023)',
    imgTit: require('../../../../../media/img/notification_file_25px.png'),
    post: 'Người đăng: nhuntq20',
    imgND: require('../../../../../media/img/teacher_HT_25px.png'),
    date: 'Thời gian: 20/07/2023 11:47',
    imgDate: require('../../../../../media/img/time_25px.png'),
  },
  {
    title:
      'THÔNG BÁO VỀ VIỆC NHẬN BẰNG TỐT NGHIỆP (ĐỢT TỐT NGHIỆP THÁNG 06/2023)',
    imgTit: require('../../../../../media/img/notification_file_25px.png'),
    post: 'Người đăng: nhuntq20',
    imgND: require('../../../../../media/img/teacher_HT_25px.png'),
    date: 'Thời gian: 20/07/2023 11:47',
    imgDate: require('../../../../../media/img/time_25px.png'),
  },
  {
    title:
      'THÔNG BÁO VỀ VIỆC NHẬN BẰNG TỐT NGHIỆP (ĐỢT TỐT NGHIỆP THÁNG 06/2023)',
    imgTit: require('../../../../../media/img/notification_file_25px.png'),
    post: 'Người đăng: nhuntq20',
    imgND: require('../../../../../media/img/teacher_HT_25px.png'),
    date: 'Thời gian: 20/07/2023 11:47',
    imgDate: require('../../../../../media/img/time_25px.png'),
  },
];
